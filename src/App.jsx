...
function computeSettlement(game, roster) {
  const players = game.playerIds.map((pid) => {
    ...
    if (balance > 0) {
      // El cash out primero salda el buy-in virtual (si lo hay); lo que sobra se compara
      // contra el buy-in en cash: al menos ese monto se entrega en cash, el resto (que es
      // exactamente el balance) se cobra por transferencia.
      const afterVirtual = cashOut - virtualAmount;
      if (virtualAmount > 0 && afterVirtual < 0) {
        // Caso defensivo: ni siquiera alcanza para saldar el virtual -> balance negativo real.
        pagoCash = 0;
      ...
      // Balance en cero o negativo: no recibe cash, todo se liquida por transferencia.
      pagoCash = 0;
  ...
  // Transferencias sugeridas: se emparejan los que más reciben con los que más deben,
  // para minimizar el número de transacciones. Como en conjunto los perdedores deben un
  // poco más de lo que los ganadores reciben (la diferencia es el rake), todos los
  // ganadores quedan cubiertos y ese remanente del rake simplemente no se transfiere a nadie.
